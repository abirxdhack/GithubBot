...
async def send_message(chat_id, text, parse_mode="html", buttons=None,
                       reply_to=None, link_preview=False, silent=None,
                       background=None, formatting_entities=None,
                       clear_draft=False, schedule=None):
    try:
...
async def edit_message(chat_id, message_id, text, parse_mode="html",
    ...
    try:
        ...
        LOGGER.warning(f"Invalid message ID editing in {chat_id}: {e}")
        return None
    except FloodWaitError as e:
        LOGGER.warning(f"FloodWait {e.seconds}s editing in {chat_id}")
        return None
    except Exception as e:
        LOGGER.error(f"edit_message in {chat_id} failed: {e}")
        return None
...
async def delete_messages(chat_id, message_ids, revoke=True):
    try:
        ...
        await _client().delete_messages(entity=chat_id, message_ids=message_ids, revoke=revoke)
        return True
    except FloodWaitError as e:
        LOGGER.warning(f"FloodWait {e.seconds}s deleting in {chat_id}")
        return False
    except Exception as e:
        LOGGER.error(f"delete_messages in {chat_id} failed: {e}")
        return False


async def send_file(chat_id, file, caption=None, parse_mode="html",
                    ...
                    clear_draft=False, schedule=None, ttl=None):
    try:
        ...
        LOGGER.warning(f"FloodWait {e.seconds}s sending file to {chat_id}")
        return None
    except (ChatWriteForbiddenError, UserIsBlockedError) as e:
        LOGGER.warning(f"Cannot send file to {chat_id}: {e}")
        return None
    except Exception as e:
        LOGGER.error(f"send_file to {chat_id} failed: {e}")
        return None


async def get_messages(chat_id, message_ids):
    try:
        return await _client().get_messages(entity=chat_id, ids=message_ids)
    except Exception as e:
        LOGGER.error(f"get_messages from {chat_id} failed: {e}")
        return None
...
async def forward_messages(to_chat, messages, from_chat, silent=None,
    ...
    try:
        if isinstance(messages, int):
            messages = [messages]
        return await _client().forward_messages(
            entity=to_chat,
            messages=messages,
            from_peer=from_chat,
            silent=silent,
        ...
        LOGGER.warning(f"FloodWait {e.seconds}s forwarding to {to_chat}")
        return None
    except Exception as e:
        LOGGER.error(f"forward_messages to {to_chat} failed: {e}")
        return None
